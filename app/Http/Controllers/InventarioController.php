...
public function import() 
    ...
    public function ReporteInventarioGeneral($fecha1, $fecha2)
    {
        $inventarios = DB::select("SELECT inventarios.id, estado, municipio, centro_salud, servicio_medico, piso, equipo, marca, modelo, serial, numero_bien_nacional, proveedor, estatu_equipo, observacion, reparado, fecha_instalacion, proveedor_servicio, name, inventarios.created_at 
            FROM inventarios
            LEFT JOIN estados ON estado_id = estados.id
            ...
            LEFT JOIN marcas ON inventarios.marca_id = marcas.id
            LEFT JOIN users ON user_id = users.id
            ORDER BY inventarios.id DESC", [$fecha1, $fecha2]);
    ...
    public function ReporteInventarioEstadal($estado_id, $fecha1, $fecha2)
    {
        $inventarios = DB::select("SELECT inventarios.id, estado, municipio, centro_salud, servicio_medico, piso, equipo, marca, modelo, serial, numero_bien_nacional, proveedor, estatu_equipo, observacion, reparado, fecha_instalacion, proveedor_servicio, name, inventarios.created_at 
            FROM inventarios
            LEFT JOIN estados ON estado_id = estados.id
            ...
            LEFT JOIN marcas ON inventarios.marca_id = marcas.id
            LEFT JOIN users ON user_id = users.id
            WHERE inventarios.estado_id = ? AND DATE(inventarios.created_at) BETWEEN  ? AND ?
            ORDER BY inventarios.id DESC", [$estado_id, $fecha1, $fecha2] );

        echo json_encode($inventarios); // para pasar en json
    }


    public function ReporteInventarioMunicipal($municipio_id, $fecha1, $fecha2 )
    {
        $inventarios = DB::select("SELECT inventarios.id, estado, municipio, centro_salud, servicio_medico, piso, equipo, marca, modelo, serial, numero_bien_nacional, proveedor, estatu_equipo, observacion, reparado, fecha_instalacion, proveedor_servicio, name, inventarios.created_at 
            FROM inventarios
            LEFT JOIN estados ON estado_id = estados.id
            ...
            LEFT JOIN marcas ON inventarios.marca_id = marcas.id
            LEFT JOIN users ON user_id = users.id
            WHERE inventarios.municipio_id = ? AND DATE(inventarios.created_at) BETWEEN  ? AND ?
            ORDER BY inventarios.id DESC", [$municipio_id, $fecha1, $fecha2] );

        echo json_encode($inventarios); // para pasar en json
    }

    public function ReporteInventarioCentroSalud($centro_salud_id, $fecha1, $fecha2)
    {
        $inventarios = DB::select("SELECT inventarios.id, estado, municipio, centro_salud, servicio_medico, piso, equipo, marca, modelo, serial, numero_bien_nacional, proveedor, estatu_equipo, observacion, reparado, fecha_instalacion, proveedor_servicio, name, inventarios.created_at 
            FROM inventarios
            LEFT JOIN estados ON estado_id = estados.id
            ...
            LEFT JOIN marcas ON inventarios.marca_id = marcas.id
            LEFT JOIN users ON user_id = users.id
            WHERE inventarios.centro_salud_id = ? AND DATE(inventarios.created_at) BETWEEN  ? AND ?
            ORDER BY inventarios.id DESC", [$centro_salud_id, $fecha1, $fecha2] );
      ...
      public function ReporteInventarioGeneralPorEstatus($equipo,$estatu_equipo_id, $fecha1, $fecha2)
      {

        $equipo = '%'.$equipo.'%';  
          $inventarios = DB::select("SELECT  historico_estatus.inventario_id, centro_salud, estado, servicio_medico, equipo, marca, serial, numero_bien_nacional, reparado, estatu_equipo, historico_estatus.created_at
          ...
          LEFT JOIN equipos ON inventarios.equipo_id = equipos.id
          LEFT JOIN marcas ON inventarios.marca_id = marcas.id
          WHERE equipos.equipo LIKE ? AND inventarios.estatu_equipo_id = ? AND DATE(historico_estatus.created_at) BETWEEN  ? AND ?
         ORDER BY inventarios.id DESC", [$equipo, $estatu_equipo_id, $fecha1, $fecha2]);
      ...
      public function ReporteInventarioEstadalPorEstatus($equipo, $estado_id, $estatu_equipo_id, $fecha1, $fecha2)
      {
        ...
        $equipo = '%'.$equipo.'%';  
          ...
          LEFT JOIN equipos ON inventarios.equipo_id = equipos.id
          LEFT JOIN marcas ON inventarios.marca_id = marcas.id
          WHERE equipos.equipo LIKE ? AND inventarios.estado_id = ?  AND inventarios.estatu_equipo_id = ? AND DATE(historico_estatus.created_at) BETWEEN  ? AND ?
         ORDER BY inventarios.id DESC", [$equipo, $estado_id, $estatu_equipo_id, $fecha1, $fecha2]);
  
          echo json_encode($inventarios); // para pasar en json
      }
  
  
      public function ReporteInventarioMunicipalPorEstatus($equipo, $municipio_id, $estatu_equipo_id, $fecha1, $fecha2)
      {
        $equipo = '%'.$equipo.'%'; 
          $inventarios = DB::select("SELECT  historico_estatus.inventario_id, centro_salud, estado, servicio_medico, equipo, marca, serial, numero_bien_nacional, reparado, estatu_equipo, historico_estatus.created_at
          FROM historico_estatus
          ...
          LEFT JOIN equipos ON inventarios.equipo_id = equipos.id
          LEFT JOIN marcas ON inventarios.marca_id = marcas.id
              WHERE equipos.equipo LIKE ? AND inventarios.municipio_id = ? AND inventarios.estatu_equipo_id = ? AND DATE(historico_estatus.created_at) BETWEEN  ? AND ?
              ORDER BY inventarios.id DESC", [$equipo ,$municipio_id, $estatu_equipo_id, $fecha1, $fecha2] );
  
          echo json_encode($inventarios); // para pasar en json
      }
  
      public function ReporteInventarioCentroSaludPorEstatus($equipo ,$centro_salud_id, $estatu_equipo_id, $fecha1, $fecha2)
      {
        $equipo = '%'.$equipo.'%'; 
          $inventarios = DB::select("SELECT  historico_estatus.inventario_id, centro_salud, estado, servicio_medico, equipo, marca, serial, numero_bien_nacional, reparado, estatu_equipo, historico_estatus.created_at
          FROM historico_estatus
          ...
          LEFT JOIN equipos ON inventarios.equipo_id = equipos.id
          LEFT JOIN marcas ON inventarios.marca_id = marcas.id
              WHERE equipos.equipo LIKE ? AND inventarios.centro_salud_id = ? AND inventarios.estatu_equipo_id = ? AND DATE(historico_estatus.created_at) BETWEEN  ? AND ?
              ORDER BY inventarios.id DESC", [$equipo ,$centro_salud_id, $estatu_equipo_id, $fecha1, $fecha2] );
       ...
       public function ReporteInventarioEstadalPorReparados($equipo ,$estado_id, $reparado, $fecha1, $fecha2)
       {
           $equipo = '%'.$equipo.'%'; 
           ...
           LEFT JOIN equipos ON inventarios.equipo_id = equipos.id
           LEFT JOIN marcas ON inventarios.marca_id = marcas.id
               WHERE equipos.equipo LIKE ? AND inventarios.estado_id = ? AND inventarios.reparado = ? AND DATE(historico_estatus.created_at) BETWEEN  ? AND ?
               ORDER BY inventarios.id DESC", [$equipo,$estado_id, $reparado, $fecha1, $fecha2] );
   
           echo json_encode($inventarios); // para pasar en json
       }
   
   
       public function ReporteInventarioMunicipalPorReparados($equipo,$municipio_id, $reparado, $fecha1, $fecha2)
       {
             $equipo = '%'.$equipo.'%'; 
           $inventarios = DB::select("SELECT  historico_estatus.inventario_id, centro_salud, servicio_medico, equipo, marca, serial, numero_bien_nacional, reparado, estatu_equipo, historico_estatus.created_at
           FROM historico_estatus
           ...
           LEFT JOIN equipos ON inventarios.equipo_id = equipos.id
           LEFT JOIN marcas ON inventarios.marca_id = marcas.id
               WHERE equipos.equipo LIKE ? AND inventarios.municipio_id = ? AND inventarios.reparado = ? AND DATE(historico_estatus.created_at) BETWEEN  ? AND ?
               ORDER BY inventarios.id DESC", [$equipo,$municipio_id, $reparado, $fecha1, $fecha2] );
   
           echo json_encode($inventarios); // para pasar en json
       }
   
       public function ReporteInventarioCentroSaludPorReparados($equipo,$centro_salud_id, $reparado, $fecha1, $fecha2)
       {
            $equipo = '%'.$equipo.'%';  
           $inventarios = DB::select("SELECT historico_estatus.inventario_id, centro_salud, servicio_medico, equipo, marca, serial, numero_bien_nacional, reparado, estatu_equipo, historico_estatus.created_at
           FROM historico_estatus
           ...
           LEFT JOIN equipos ON inventarios.equipo_id = equipos.id
           LEFT JOIN marcas ON inventarios.marca_id = marcas.id
               WHERE equipos.equipo LIKE ? AND inventarios.centro_salud_id = ? AND inventarios.reparado = ? AND DATE(historico_estatus.created_at) BETWEEN  ? AND ?
               ORDER BY inventarios.id DESC", [$equipo, $centro_salud_id, $reparado, $fecha1, $fecha2] );
       ...
       public function ReporteInventarioBuscarEquipo($equipo, $fecha1, $fecha2)
       {
           $equipo = '%'.$equipo.'%';
           $inventarios = DB::select("SELECT inventarios.id, estado, municipio, centro_salud, servicio_medico, piso, equipo, marca, modelo, serial, numero_bien_nacional, proveedor, estatu_equipo, observacion, reparado, fecha_instalacion, proveedor_servicio, name, inventarios.created_at 
               FROM inventarios
               ...
               LEFT JOIN marcas ON inventarios.marca_id = marcas.id
               LEFT JOIN users ON user_id = users.id
               WHERE equipos.equipo LIKE ? AND DATE(inventarios.created_at) BETWEEN  ? AND ?
               ORDER BY inventarios.id DESC", [$equipo, $fecha1, $fecha2] );
       ...
       public function ReporteInventarioBuscarEquipoEstados($equipo, $estado_id, $fecha1, $fecha2)
       {
           $equipo = '%'.$equipo.'%';
           $inventarios = DB::select("SELECT inventarios.id, estado, municipio, centro_salud, servicio_medico, piso, equipo, marca, modelo, serial, numero_bien_nacional, proveedor, estatu_equipo, observacion, reparado, fecha_instalacion, proveedor_servicio, name, inventarios.created_at 
               FROM inventarios
               ...
               LEFT JOIN marcas ON inventarios.marca_id = marcas.id
               LEFT JOIN users ON user_id = users.id
               WHERE equipos.equipo LIKE ? AND inventarios.estado_id = ? AND DATE(inventarios.created_at) BETWEEN  ? AND ?
               ORDER BY inventarios.id DESC", [$equipo, $estado_id, $fecha1, $fecha2] );
   
           echo json_encode($inventarios); // para pasar en json
       }


       public function ReporteInventarioBuscarEquipoMunicipios($equipo, $municipio_id, $fecha1, $fecha2)
       {
           $equipo = '%'.$equipo.'%';
           $inventarios = DB::select("SELECT inventarios.id, estado, municipio, centro_salud, servicio_medico, piso, equipo, marca, modelo, serial, numero_bien_nacional, proveedor, estatu_equipo, observacion, reparado, fecha_instalacion, proveedor_servicio, name, inventarios.created_at 
               FROM inventarios
               ...
               LEFT JOIN marcas ON inventarios.marca_id = marcas.id
               LEFT JOIN users ON user_id = users.id
               WHERE equipos.equipo LIKE ? AND inventarios.municipio_id = ? AND DATE(inventarios.created_at) BETWEEN  ? AND ?
               ORDER BY inventarios.id DESC", [$equipo, $municipio_id, $fecha1, $fecha2] );
   
           echo json_encode($inventarios); // para pasar en json
       }


       public function ReporteInventarioBuscarEquipoCentroSalud($equipo, $centro_salud_id, $fecha1, $fecha2)
       {
           $equipo = '%'.$equipo.'%';
           $inventarios = DB::select("SELECT inventarios.id, estado, municipio, centro_salud, servicio_medico, piso, equipo, marca, modelo, serial, numero_bien_nacional, proveedor, estatu_equipo, observacion, reparado, fecha_instalacion, proveedor_servicio, name, inventarios.created_at 
               FROM inventarios
               ...
               LEFT JOIN marcas ON inventarios.marca_id = marcas.id
               LEFT JOIN users ON user_id = users.id
               WHERE equipos.equipo LIKE ? AND inventarios.centro_salud_id = ? AND DATE(inventarios.created_at) BETWEEN  ? AND ?
               ORDER BY inventarios.id DESC", [$equipo, $centro_salud_id, $fecha1, $fecha2] );
 ...
 // REPORTES GENERAL GLOBALES //
       ...
       public function ReporteInventarioGeneralEquiposGlobal($equipo,$fecha1, $fecha2)
       {
        $equipo = '%'.$equipo.'%';
           $inventarios = DB::select("SELECT e.estado,
           (SELECT count(estatu_equipo_id) FROM inventarios WHERE estatu_equipo_id in (1) AND e.id=estado_id) AS equipos_operativos,
           ...
           WHERE equipos.equipo LIKE ? AND DATE(i.created_at) BETWEEN ? AND ?
           GROUP BY e.estado, equipos_operativos, equipos_inoperativos
           ORDER BY e.estado", [ $equipo ,$fecha1, $fecha2] );
   
           echo json_encode($inventarios); // para pasar en json
       }


       public function ReporteInventarioEstadalEquiposGlobal($equipo, $estado_id ,$fecha1, $fecha2)
       {
        $equipo = '%'.$equipo.'%';
           $inventarios = DB::select("SELECT e.estado,
           (SELECT count(estatu_equipo_id) FROM inventarios WHERE estatu_equipo_id in (1) AND e.id=estado_id) AS equipos_operativos,
           ...
           INNER JOIN estatu_equipos AS h on h.id=i.estatu_equipo_id
           LEFT JOIN equipos ON i.equipo_id = equipos.id
           WHERE equipos.equipo LIKE ? AND i.estado_id = ?  AND DATE(i.created_at) BETWEEN ? AND ?
           GROUP BY e.estado, equipos_operativos, equipos_inoperativos
           ORDER BY e.estado", [ $equipo ,$estado_id ,$fecha1, $fecha2] );
   
           echo json_encode($inventarios); // para pasar en json
       }


       public function ReporteInventarioMunicipiosEquiposGlobal($equipo, $municipio_id ,$fecha1, $fecha2)
       {
        $equipo = '%'.$equipo.'%';
           $inventarios = DB::select("SELECT e.estado, m.municipio,
           (SELECT count(estatu_equipo_id) FROM inventarios WHERE estatu_equipo_id in (1) AND e.id=estado_id) AS equipos_operativos,
           ...
           INNER JOIN municipios AS m on i.municipio_id=m.id
           INNER JOIN centro_salud as c ON i.centro_salud_id = c.id
           WHERE equipos.equipo LIKE ? AND i.municipio_id = ?  AND DATE(i.created_at) BETWEEN ? AND ?
           GROUP BY e.estado, equipos_operativos, equipos_inoperativos
           ORDER BY e.estado", [ $equipo ,$municipio_id ,$fecha1, $fecha2] );
   
           echo json_encode($inventarios); // para pasar en json
       }

       public function ReporteInventarioCentroSaludEquiposGlobal($equipo, $centro_salud_id ,$fecha1, $fecha2)
       {
        $equipo = '%'.$equipo.'%';
           $inventarios = DB::select("SELECT e.estado, m.municipio, c.centro_salud,
           (SELECT count(estatu_equipo_id) FROM inventarios WHERE estatu_equipo_id in (1) AND e.id=estado_id) AS equipos_operativos,
           ...
           INNER JOIN municipios AS m on i.municipio_id=m.id
           INNER JOIN centro_salud as c ON i.centro_salud_id = c.id
           WHERE equipos.equipo LIKE ? AND i.centro_salud_id = ? AND DATE(i.created_at) BETWEEN ? AND ?
           GROUP BY e.estado, equipos_operativos, equipos_inoperativos
           ORDER BY e.estado", [ $equipo ,$centro_salud_id ,$fecha1, $fecha2] );
